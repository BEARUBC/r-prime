...
async fn get_battery_levels(contacts: Port<MS>) -> SystemJob {
...
async fn get_humidity_levels(_: Port<MS>) -> SystemJob {
    // doing a bunch of GPIO interfacing
...
async fn handler1(_: Port<MS>, _: MS) { println!("got a message") }
async fn handler2(_: Port<MS>, _: MS) { println!("got a message") }
